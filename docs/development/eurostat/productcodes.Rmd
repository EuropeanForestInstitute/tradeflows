...
```{r eval=FALSE}
con <- RMySQL::dbConnect(RMySQL::MySQL(), dbname = "test")
inputfields <- RMySQL::dbListFields(con,"raw_comext_cn")
outputfields <- RMySQL::dbListFields(con,"val_comext_cn")
# validated fields should exist in the raw table
stopifnot(outputfields %in% inputfields)

# clean products:
cleancode(con, "raw_comext_cn", "val_comext_cn", productcode)
