...
The [UN Comtrade API](http://comtrade.un.org/data/doc/api/) 
...
## Classification
```{r}
load("data-raw/classificationH4.RData")  
classificationH4 %>% filter(id=="4407")  %>% kable
classificationH4 %>% filter(parent=="4407")  %>% kable
classificationH4 %>% filter(id=="44")  %>% kable
classificationH4 %>% filter(parent=="44") %>% kable
# Number of products under chapter 44
chap44 <- classificationH4 %>% filter(substr(id, 0, 2)=="44")
nrow(chap44)
