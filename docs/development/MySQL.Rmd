...
```{bash eval=FALSE}
# Make sure the archive doesn't exist already 
# other wise it will contain the SQL dump twice
# prefer 7zr to 7z as this is the only one available on the server
mysqldump tradeflows raw_comext_monthly_201707 | 7zr a -si raw_comext_monthly_201707.sql.7z
```
...
RMySQL::dbDataType(con, 1)
RMySQL::dbDataType(con, 1L)
RMySQL::dbDataType(con, Sys.Date())
