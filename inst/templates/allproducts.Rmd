...
######### #
# ! This chunk is used for development only,
# remove it or change eval=FALSE.
######### #
...
#----------------------------------------------------------
message("Using the package template in ",
        'inputpath = system.file("templates", package="tradeflows")')
message("Build the package to update this package template")
createcountryreport(country)
```
...
```{r eval=FALSE}
# old product description based on the itto table
#---------------------------------------------------
...
#' Use:
#' description(c(440799, 440795))
description_from_itto_table <- function(productcodes){
    message("the issue is that long descriptions were truncated in this table")
    descr <- productitto %>% 
        filter(productcode %in% productcodes) %>% 
